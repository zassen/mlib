...
//INFO_SET_LEVEL(INFO_LEVEL_INFO);
...
private:
	...
	virtual int handleEvent(int fd, int events, void* data){
		...
		ssize_t nRead;
		ssize_t count=0;
		INFO("ENTER callback handler fd=%d, event =%d",fd,events);
		do{
		nRead = read(fd, buffer, sizeof(buffer));
		count += nRead;
		}while((nRead == -1 && errno == EINTR) || nRead == sizeof(buffer));
		INFO(" callback handler count = %ld,data=%s ", count, buffer);
	}
